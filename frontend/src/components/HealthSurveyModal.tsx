// Substitua o conteúdo em: src/components/HealthSurveyModal.tsx
...
const ModalStep1: React.FC<ModalStep1Props> = ({ onNext }) => {
  ...
  const handleFileButtonClick = () => {
    fileInputRef.current?.click(); // Simula o clique no input de arquivo escondido
  };
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      ...
      // Aqui você faria o upload do arquivo para o backend
    ...
    <>
      <div className="flex justify-between items-center mb-8"> {/* Espaçamento maior */}
        ...
        <input
          ...
          accept=".pdf,.png,.jpg,.jpeg" // Aceita PDF e imagens
          onChange={handleFileChange}
        />
        <button
          type="button"
          onClick={handleFileButtonClick} // Clica no input escondido
      ...
      <form className="space-y-4"> {/* Espaçamento entre os inputs */}
...
const ModalStep2: React.FC<ModalStep2Props> = ({ onFinish }) => (
  <>
    <h2 className="text-3xl font-bold text-gray-900 mb-8">Sistema de Pontuação</h2> {/* Cor do texto */}

    <form className="space-y-4"> {/* Espaçamento entre os inputs */}
      {/* Selects com novo estilo */}
      <div className="relative">
        <select className="w-full p-4 bg-white border border-gray-300 rounded-xl text-gray-900 appearance-none focus:outline-none focus:ring-2 focus:ring-indigo-500 pr-10">
          <option value="" disabled selected>Medicamentos</option> {/* Placeholder */}
...
export const HealthSurveyModal: React.FC<HealthSurveyModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      {/* Conteúdo do Modal - Fundo BRANCO, bordas mais arredondadas (rounded-3xl) e mais largo (max-w-xl) */}
      <div
        onClick={(e) => e.stopPropagation()}
        className="relative bg-white shadow-2xl rounded-3xl w-full max-w-xl p-8 md:p-10" // Padding maior
      >
        {/* Botão de Fechar (X) - Cor ajustada para tema claro */}
        <button
          onClick={onClose}
